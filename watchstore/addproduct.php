...
<body>
            ...
            <select class="inp1" name="brand" id="brand" required>
                ...
                if ($brand_result && mysqli_num_rows($brand_result) > 0) {
                    while ($brand = mysqli_fetch_assoc($brand_result)) {
                        echo "<option value='" . $brand['brandid'] . "'>" . htmlspecialchars($brand['brandname']) . "</option>";
                    }
                } else {
    ...
    if (isset($_POST['addwatch'])) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
            ...
            $insert = "INSERT INTO products (model, price, quantity, image, pdiscription, brand) VALUES ('$model', '$price', '$qnty', '$image', '$detail', '$brand_id')";
